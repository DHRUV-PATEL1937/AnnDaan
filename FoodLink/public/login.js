// Neumorphism Login Form JavaScript (Corrected & Refactored)
class NeumorphismLoginForm {
  ...
  bindEvents() {
    ...
    this.usernameForm.addEventListener("submit", (e) =>
      this.handleUsernameSubmit(e)
    );
    this.modalUsernameInput.addEventListener("input", () =>
  ...
  validateEmail() {
    ...
    if (!emailRegex.test(email))
      return this.showError("email", "Please enter a valid email");
  ...
  async handleUsernameSubmit(e) {
    ...
    const selectedRole =
      document.querySelector('input[name="modal-role"]:checked')?.value ||
      "user";
    ...
    if (!usernameRegex.test(username)) {
      this.showModalError(
        "Invalid format (3-20 letters, numbers, or underscores)."
      );
      return;
    }
  ...
  showSuccessAndRedirect(redirectUrl) {
    ...
    const formElements = loginCard.querySelectorAll(
      ".login-header, .login-form, .divider, .google-signin, .signup-link"
    );
  ...
  _getRedirectUrl(role) {
    switch (role) {
      case "ngo":
        return "ngo_dashboard.html";
      case "rider":
        return "rider_dashboard.html";
      case "user":
      case "donator":
      default:
...
// This is the callback function for Google Sign-In.
// It ensures the form is initialized and then passes the response to the class instance.
